...
(function ($) {
  ...
  window.tpgFixLazyLoad = function () {
    $(".rt-tpg-container").each(function () {
      $(this)
        .find("img.jetpack-lazy-image:not(.jetpack-lazy-image--handled)")
        .each(function () {
          $(this)
            .addClass("jetpack-lazy-image--handled")
            .removeAttr("srcset")
            .removeAttr("data-lazy-src")
            .attr("data-lazy-loaded", 1);
        });
      $(this)
        .find("img.lazyload")
        .each(function () {
        ...
        if (isIsotop.length) {
          ...
          IsotopeWrap = isIsotop.imagesLoaded(function () {
            ...
            IsotopeWrap.isotope({
              ...
              filter: function () {
                var $this = $(this);
                var searchResult = qsRegex ? $this.text().match(qsRegex) : !0;
                var buttonResult = buttonFilter ? $this.is(buttonFilter) : !0;
  ...
  function windowHashChange(isotope, IsoButton) {
    ...
    isotope.isotope({ filter: $hashFilter });
    IsoButton.find("button").removeClass("selected");
    IsoButton.find('button[data-filter="' + $hashFilter + '"]').addClass(
      "selected"
    );
  }
  function decodeHash() {
  ...
  function isoFilterCounter(container, isotope) {
    ...
    container
      .find('.rt-tpg-isotope-buttons button[data-filter="*"]')
      .find("span")
      .remove();
    container
      .find('.rt-tpg-isotope-buttons button[data-filter="*"]')
  ...
  function HeightResize() {
    ...
    function equalHeight4Layout4() {
      ...
      $(
        ".rt-row.rt-content-loader.layout4 .layout4item .layoutInner .rt-img-holder img,.rt-row.rt-content-loader.layout4 .layout4item .layoutInner.layoutInner-content"
      ).height($maxH + "px");
    }
  }
      ...
      if (widgets.length > 0) {
        for (var i = 0; i < widgets.length; i++) {
          if (widgets[i].contains(e.target)) {
            ...
            if (
              icon.classList.toString().indexOf("tss-promotional-element") >= 0
            ) {
              ...
              if (dialog.querySelector(".rt-dialog-buttons-action") === null) {
                ...
                button.setAttribute(
                  "href",
                  "//www.radiustheme.com/downloads/the-post-grid-pro-for-wordpress/"
                );
